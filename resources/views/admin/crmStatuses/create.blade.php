...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-statuses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="reserva">{{ trans('cruds.crmStatus.fields.reserva') }}</label>
                <input class="form-control {{ $errors->has('reserva') ? 'is-invalid' : '' }}" type="text" name="reserva" id="reserva" value="{{ old('reserva', '#') }}" required>
                @if($errors->has('reserva'))
                    <span class="text-danger">{{ $errors->first('reserva') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.crmStatus.fields.reserva_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('companhia') ? 'is-invalid' : '' }}" name="companhia" id="companhia" required>
                    ...
                    @foreach(App\Models\CrmStatus::COMPANHIA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('companhia', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('origem') ? 'is-invalid' : '' }}" name="origem" id="origem" required>
                    ...
                    @foreach(App\Models\CrmStatus::ORIGEM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('origem', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('destino') ? 'is-invalid' : '' }}" name="destino" id="destino" required>
                    ...
                    @foreach(App\Models\CrmStatus::DESTINO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('destino', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cc') ? 'is-invalid' : '' }}" type="text" name="cc" id="cc" value="{{ old('cc', '') }}" required>
                @if($errors->has('cc'))
